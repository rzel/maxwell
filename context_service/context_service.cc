...
class ContextServiceImpl : public ContextService {
 public:
  ...
  void RegisterAcquirer(const mojo::String& whoami,
                        InterfaceRequest<ContextAcquirerClient> client)
                        override {
    caq_clients_.AddBinding(new ContextAcquirerClientImpl(whoami, &repo_),
                            client.Pass());
  }

  void RegisterContextAgent(const mojo::String& whoami,
                            InterfaceRequest<ContextAgentClient> client)
                            override {
    cag_clients_.AddBinding(new ContextAgentClientImpl(whoami, &repo_),
                            client.Pass());
  }

  void RegisterSuggestionAgent(const mojo::String& whoami,
                               InterfaceRequest<SuggestionAgentClient> client)
                               override {
...
class ContextServiceApp : public ApplicationImplBase {
 public:
  ...
  bool OnAcceptConnection(ServiceProviderImpl* service_provider_impl) override {
    // Singleton service
    service_provider_impl->AddService<ContextService>(
        [this](const ConnectionContext& connection_context,
               InterfaceRequest<ContextService> request) {
 ...
 private:
  ...
  BindingSet<ContextService> bindings_;
